...
public class RedisLoader implements SlimeLoader {
    ...
    public RedisLoader(String uri) {
        this.connection = RedisClient.create(uri)
                .connect(StringByteCodec.INSTANCE)
                .sync();
    }

    @Override
    public byte[] readWorld(String name) throws UnknownWorldException {
    ...
    public List<String> listWorlds() {
        return connection.smembers(WORLD_LIST_PREFIX).stream()
                .map(bytes -> new String(bytes, StandardCharsets.UTF_8))
                .collect(Collectors.toList()); // We can't use .toList because this needs to be mutable.
    }

    @Override
    public void saveWorld(String worldName, byte[] bytes) {
